...
@st.cache_data
def load_data():
    metric_1 = pd.read_csv('cbbwinshares.csv')
    return metric_1
...
# Main app structure
st.title("NCAA Player Rating Dashboard")
...
num_teams = st.slider("Select the number of top teams to display", min_value=1, max_value=14, value=10)
...
num_players = st.slider("Select the number of top players to display", min_value=1, max_value=30, value=10)
